...
- **I/O wait is a SEPARATE floor, and unlike rAF pacing it is latency, so it is hideable.** Do not collapse the two under "floor-bound" and conclude there is nothing left to do. `disableAutoFetch: true` makes every byte a sequential demand fetch: a cold jump to page 138 issued **196 range requests for 34MB, 3463ms of measured I/O**, each a full IPC round trip, pdf.js parsing a little and then waiting again. Prefetching the destination's ranges on navigation, or widening request granularity, would attack it without touching `disableAutoFetch` (which is load-bearing — see above). Untried; the `io` counters exist for it.
...
**Summed render timings are NOT additive** — and this applies to `io` too, which is the trap catching us twice. Range requests run up to **60 concurrent** (measured), so summing 196 elapsed durations produced "3463ms of I/O" for work whose real transport cost is **~123ms**: measured at **0.225ms per message + 2.962ms/MB**, so 196 messages plus 26.6MB. That bogus 3463ms was quoted as a finding in two reports and a roadmap entry before anyone checked concurrency. **Before treating any summed duration as cost, measure how many of them overlap.**
