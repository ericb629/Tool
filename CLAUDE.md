...
**Viewer state.** The PDF viewer renders tiled and sharp at every zoom, holds recently-unmounted pages in a bounded LRU so navigation does not re-parse, and is performance-closed: zoom, pan and page revisits are fast, and what remains slow is the *first* visit to an image-heavy exhibit sheet (~8s, versus ~1.7s for a normal plan sheet and ~350ms for any revisit). That residual cost is split between an unfixable floor — pdf.js paces rendering through `requestAnimationFrame` — and an addressable one, sequential demand I/O under `disableAutoFetch`. Flag-gated timing instrumentation is retained on purpose; see `docs/VIEWER_PERF_ROADMAP.md` for what shipped, what was measured and rejected, and how to turn the counters on.
...
- **EVERY pdf.js runtime asset must be reachable at a real URL in BOTH dev and packaged, and dev passing tells you nothing about packaged.** This is a pattern, not a one-off. The assets are: the **worker** (`pdf.worker.mjs`), the **wasm codecs** (`jbig2.wasm` for JBIG2, `openjpeg.wasm` for JPEG2000), **cMaps** (CID fonts), **standard font data** (the 14 base fonts) — and whatever a future pdfjs-dist version adds. Dev serves them over `http://localhost`; packaged serves them over `file://` *inside `app.asar`*, and the two resolve differently.
  - **Why it bites silently:** a missing codec does not throw. pdf.js **warns and omits the image** — pages 4 and 11–15 of the Kincora set are single scanned JBIG2 images, so they rendered as near-empty sheets that still accepted calibration and takeoff. That is the wrong-number-that-looks-plausible class, not a cosmetic bug.
  - **The working mechanism, reuse it:** `scripts/copy-pdfjs-assets.mjs` copies `wasm/`, `cmaps/` and `standard_fonts/` out of `node_modules/pdfjs-dist` into the renderer's `publicDir` (gitignored, so they stay version-locked rather than going stale as checked-in binaries), wired via the `predev`/`prebuild` npm hooks. URLs are built from `document.baseURI` in `pdf/pdfjs.ts` so one expression works in both builds, and **all three need a trailing slash** — `getFactoryUrlProp` throws without one. `dist` must invoke `npm run build` (not `electron-vite build` directly) or the `prebuild` hook never fires and the package ships without the assets.
  - **Measured, so don't re-derive it:** `asarUnpack` is **not** needed — `file://` reads from inside `app.asar` work for these, verified by rendering page 4 in a packaged build. The pdf.js worker is also a **real worker thread** in the packaged build; `#setupFakeWorker` does not fire.
  - **When adding a codec or upgrading pdfjs-dist:** add the directory to `ASSETS` in the copy script, and verify in a **packaged** build, not dev. `pdf/decodeAudit.ts` will flag a page whose images failed to decode, so a regression surfaces in the UI rather than as a quietly blank sheet.
- **Do not reintroduce a custom protocol scheme for PDF DATA.** pdf.js hard-codes incremental loading to `http(s)` (`isHttp = /https?:/.test(url.protocol)`), so any custom scheme silently downloads the entire document — measured at +1.2 GB.
...
- **Scroll position and scale must be consistent WITHIN a render pass.** A scale change re-renders with the new layout while `scrollTop` still holds the value from the old one, because the anchored scroll is written in a layout effect that runs afterwards. Every page box has moved, so that stale offset selects the wrong pages - and they fully mount (`getPage`, preview, tile effect) before being destroyed on the next pass. Traced on one zoom click at page 23: `range 15-18 top 13523` then, 3ms later, `range 22-24 top 19423`. **Four pages mounted and thrown away, having never been on screen.** `applyZoomAnchor` is pure, so derive the anchored offset during render and select pages from that; the DOM write can stay in the effect. This churn was the root cause of three separate symptoms - cancelled previews (the white flash), 23.4s of destroyed parses, and per-page state being wiped before it could be used - so treat "pages are remounting" as a cause to find, never a constraint to work around.
- **The "white flash" during zoom was never fixed — it was masked, and it can come back.** Record the mechanism, not the symptom: the preview renders into an off-screen canvas and blits to the visible one **only on success**, so a *cancelled* preview never blits and the visible canvas keeps its transparent default. That transparent canvas is the white. It was not "the preview is too blurry to notice" — there was no preview at all. Measured across five zoom clicks: **10 of 18 previews cancelled** before the preview gate landed, **0 after**. The gate removed the cancellations as a side effect; nothing addressed the blit-on-success-only design. **Anything that raises page churn during zoom re-opens that window** — so do not "fix" a returning flash by chasing cancellation counts. Either make a cancelled preview leave something on the canvas, or keep churn low, and know which one you are relying on.
- **A page must never be able to wait forever for its first tile.** The preview waits for the page's first tile so the sharp layer is not competing with it, and as first written that wait had no exit — `wantsTiles && !hasTile` left the page **permanently blank** if a tile never landed. Three things cause that: a tile render throwing something other than a cancellation (the loop returns and no further tiles are attempted), every tile being cancelled by a continuous gesture, and any stall further down. Only the first has an error to observe, so the gate needs a deadline as well (`pdf/previewGate.ts`). A permanently blank sheet still accepts calibration and takeoff and, unlike a decode failure, carries no banner — same failure class as the JBIG2 bug, without the tell.
...
- **Rendering is frame-paced, and this floor is STRUCTURAL.** `InternalRenderTask._scheduleNext` schedules every continuation through `requestAnimationFrame` (`pdf.mjs:16972`), so a render needing N continuations has a floor of N × ~16.7ms *regardless of machine speed*, and concurrent renders progress in lockstep — five renders finished within 2.5ms of each other after 8.2s. This is why removing four of six concurrent renders bought only 12%: raster is **not** contention-bound. Nothing short of changing how pdf.js schedules continuations moves it, so do not attack it with concurrency limits, worker pools or reordering. Corollary: **a hidden or unfocused window throttles rAF to nothing, so any measurement taken against a window that is not on screen is invalid** — it looks exactly like rendering hung.
- **I/O is NOT a floor worth attacking — this was measured and the earlier claim here was wrong.** A cold jump to page 138 issues ~196 range requests, and this bullet used to quote "3463ms of measured I/O" and recommend prefetching or wider granularity. That 3463ms was a sum of overlapping durations (peak **60** requests in flight). Measured per-operation instead — **0.225ms per message, 2.962ms/MB (~338 MB/s end to end)** — the real transport cost of that jump is **~123ms**. `rangeChunkSize` tuning (unset, so pdf.js's 64KB default), request coalescing, per-tick batching and a read-ahead buffer in main are all competing for the ~44ms of per-message overhead between them; **do not build them.** Disk is not it either: the whole 523MB file reads in 246ms, and full residency improves a page parse by ~10% for ~523MB per open document. **The far-jump cost is decode** — page 137 alone parses in 5265ms.
- **Do not assume the neighbouring page is co-visible.** Every page in the Kincora set is 2592×1728 after `/Rotate`, and a 3:2 page at fit width is taller than any landscape viewport, so a page jump leaves **exactly one page visible, at ~98% of the viewport**; the neighbour has no visible region at all and is already overscan. A plan to prioritise the primary page over "other visible pages" was written against the opposite belief and would have been inert. `test/visiblePages.test.ts` pins this, and it is cheap to re-run against a different sheet geometry — check it before reasoning about which pages compete.
- **The tile layer is TWO generations, and the stale one is load-bearing.** `tileKey` bakes in the scale, so one zoom step invalidates every tile at once. Discarding them immediately left the 250k-pixel preview stretched over the whole page — a 2.6× upscale at fit width, far worse zoomed in — so each step went sharp → very blurry → sharp, which is what "zooming feels like it's flashing" was. The outgoing generation now stays on screen underneath, re-placed by the ratio between the two scales (`scaleTileRect`), and is dropped when the new set *completes* — not on its first tile, which covers only one corner. Two rules keep it safe: **only one stale generation**, and **never promote an empty generation** (on a fast zoom the previous step may not have landed, and promoting nothing would put the blur back — the exact case it exists for). It is re-placed in a **layout** effect; a passive one leaves it a frame behind on every step. When a stale generation is on screen the new tiles are also **attached all at once**, not as they land — attaching progressively over a stretched layer is what made zooming look quilted, small sharp patches beside larger blurred ones. On arrival there is nothing underneath, so tiles attach progressively there instead. This is the "old bitmap stretched while a re-render is in flight" exception, made concrete — **it is not a CSS-scaled fake zoom and must not be deleted as one.** An earlier attempt at this did nothing at all because its refs died to the per-zoom remount bug; that was fixed in `9e18835`, and **if pages ever remount on zoom again this silently stops working.**
- **Tiling is a HIGH-ZOOM mechanism, so below `SINGLE_CANVAS_MAX_PIXELS` the page renders as ONE canvas.** Tiles exist to stay clear of the paint cliff and to bound memory when a page dwarfs the viewport — both zoom problems. At fit width the whole page is a few Mpx, and cutting it into 2–6 canvases cost N−1 redundant replays of the *entire* operator list, because **pdf.js does no per-tile culling**. That is why zooming got slower on image-heavy sheets and barely changed on simple ones: the tax scales with how expensive the content is, not with the tile count. `fitsSingleCanvas` now picks one whole-page tile when it fits in 32 Mpx of device pixels — every fit-width scale at either dpr, plus a step or two — and the grid takes over above that. A single cell is also trivially pan-stable, so it gives up nothing the grid was providing. The trade is that it rasterises off-screen page area above fit width; the budget is what bounds it. **This is not "raise `TILE_PX`"** — that proposal's ~46% figure came from the discredited `T(N) = N·D + F` model and must not be resurrected. Note `test/tiles.test.ts`'s "overlay is viewport-sized, not page-sized" now describes the **grid path only**; on the single-canvas path the overlay is page-sized and safe by the same budget.
...
**A green test suite is not evidence that a change works.** Concrete instance: the two-generation tile scheme typechecked and passed all 332 tests, and did nothing at all - its state lived in per-component refs while pages were remounting on every zoom step, so React destroyed it before it could render. The defect was a lifecycle fact, invisible to any unit test. It was caught in about a minute by counting tiles in the DOM 80ms after a zoom click. Do not commit render-path work on a green suite alone; drive it and look.
...
**Operator count is NOT a cost proxy.** Page 10 has **1,489,578** operators and parses in **3071ms**; page 137 has **12,318** and takes **5265ms**. 120x fewer operators, nearly twice as slow. **Decode dominates, not dispatch** - the expensive pages are the image-heavy ones, and their cost is JBIG2/JPEG2000 decoding, not walking the operator list. Measured across the Kincora set: vector plan sheets parse in 141-370ms, photo-collage exhibit sheets in 2429-5265ms. Anything that reasons from operator count is suspect, including the `T(N) = N*D + F` tile-tax model and its ~46% TILE_PX prediction, whose `D` was interpreted as per-canvas operator-list dispatch. **Re-derive that model before trusting it.**

**NEVER SUM ELAPSED TIMES OVER CONCURRENT OPERATIONS.** A timer measures a *window*, and N overlapping windows count the same wall clock N times, so the sum is not a cost — it is not a quantity of anything, and neither is the gap between two such sums. Use **wall clock** for "how long did the user wait" and **per-operation cost** (a controlled sweep: fixed cost per call plus marginal cost per byte/pixel) for "what would removing one save". A summed total is legitimate for ranking items *within* one phase and for nothing else. Any counter that can overlap needs a high-water companion — `io:peak in flight` exists for exactly this — and if you cannot say how many overlapped, you cannot quote the sum.

This has now produced two wrong findings, in both buckets it applies to. Raster: 41s of "raster" recorded inside an 8.3s wall clock. I/O: range requests run up to **60 concurrent** (measured), so summing 196 elapsed durations gave "3463ms of I/O" for work whose real transport cost is **~123ms** — measured at **0.225ms per message + 2.962ms/MB**, i.e. 196 messages plus 26.6MB. The bogus 3463ms was quoted in two reports and two roadmap entries, and it was still being cited as "~2.5s of pure IPC overhead" after the correction had been written down, because the retraction had not been made where the claim was. The arithmetic that should have caught it immediately: 936ms of `read()` for 26.6MB contradicts the 2.1GB/s disk rate measured four lines above it in the same document, which puts the real read at ~13ms.

**The render-timing case in detail.** Renders run concurrently (preview plus one task per tile, across up to three mounted pages) and each timer measures *elapsed* wall time, so N concurrent renders count the same window N times — 41s of "raster" was recorded inside an 8.3s wall clock. **Wall clock per phase is the only honest aggregate**; per-kind totals are useful for ranking within a phase and meaningless when summed. Related: pdf.js's own `StatTimer` keys running timers by name only, so two concurrent renders of one page corrupt each other and emit `start: undefined` → `NaN`, which poisons `total`/`max` while leaving `median` plausible-looking. `Page Request` is safe (built once per operator list); `Rendering` is not.
...
Filed behind all of the above, deliberately: a **render-priority queue** — a semaphore ordering visible-page rasterisation ahead of overscan, so off-screen previews cannot compete with the page the user navigated to. It is the correct fix for the remaining arrival cost and it is an architecture change, not a constant. Measured evidence says it is *low* priority despite being correct: removing four of six concurrent renders sped the sharp layer up by only ~12%, so contention is not the binding constraint — frame pacing and I/O waits are. Do not build it expecting a large win.
